...
from .models import Assignment, AssignmentSubmission, StudentExtra
from . import forms, models
from .forms import ClassScheduleForm
from .models import ClassSchedule,StudentExtra,TeacherExtra
...
def admin_add_schedule_view(request):
    ...
    return render(request, 'school/admin_add_schedule.html', {'form': form})
...
def admin_view_schedule_view(request):
    schedules = ClassSchedule.objects.all().order_by('date', 'time')
    return render(request, 'school/admin_view_schedule.html', {'schedules': schedules})


# Admin: Edit Schedule
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_edit_schedule_view(request, id):
    schedule = ClassSchedule.objects.get(id=id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('admin-view-schedule')
    return render(request, 'school/admin_edit_schedule.html', {'form': form})


# Admin: Delete Schedule
@login_required(login_url='adminlogin')
@user_passes_test(is_admin)
def admin_delete_schedule_view(request, id):
    schedule = ClassSchedule.objects.get(id=id)
    schedule.delete()
...
# Teacher views their scheduled classes
@login_required(login_url='teacherlogin')
@user_passes_test(is_teacher)
def teacher_view_schedule_view(request):
    teacher = TeacherExtra.objects.get(user=request.user)
    schedules = ClassSchedule.objects.filter(teacher=teacher)
    return render(request, 'school/teacher_view_schedule.html', {'schedules': schedules})


# Student views schedule for their class
@login_required(login_url='studentlogin')
@user_passes_test(is_student)
def student_view_schedule_view(request):
    student = StudentExtra.objects.get(user=request.user)
    schedules = ClassSchedule.objects.filter(cl=student.cl).order_by('date', 'time')
    return render(request, 'school/student_view_schedule.html', {'schedules': schedules})


# Assignment for teacher and student

@login_required
def post_assignment_view(request):
    if request.method == 'POST':
        form = AssignmentForm(request.POST, request.FILES)
        if form.is_valid():
            assignment = form.save(commit=False)
            assignment.teacher = TeacherExtra.objects.get(user=request.user)
            assignment.save()
            return redirect('teacher-assignment-list')  # update with your URL name
    else:
        form = AssignmentForm()
    return render(request, 'school/post_assignment.html', {'form': form})

@login_required
def teacher_assignment_list(request):
    teacher = get_object_or_404(TeacherExtra, user=request.user)
    assignments = Assignment.objects.filter(teacher=teacher)
    return render(request, 'school/teacher_assignment_list.html', {'assignments': assignments})

@login_required
def teacher_view_submissions(request, assignment_id):
    assignment = get_object_or_404(Assignment, id=assignment_id)
    submissions = AssignmentSubmission.objects.filter(assignment=assignment)
    return render(request, 'school/view_submissions.html', {'assignment': assignment, 'submissions': submissions})

@login_required
def student_assignments_view(request):
    student = get_object_or_404(StudentExtra, user=request.user)
    assignments = Assignment.objects.filter(cl=student.cl)
    return render(request, 'school/student_assignment_list.html', {'assignments': assignments})
